...
const menuLinks = document.querySelectorAll(
  ".menu__link, .menu__sublink, .top-header__buttons, .top-header__search, .top-header__logo"
)
...
if (iconMenu) {
  ...
  menuLinks.forEach((link) => {
    link.addEventListener("click", function () {
      if (iconMenu.getAttribute("aria-expanded") === "true") {
        toggleMenu()
      }
    })
  })
...
document.addEventListener("DOMContentLoaded", function () {
  document.addEventListener("click", function (e) {
    ...
    if (targetElement.closest(".bottom-footer__up")) {
      window.scrollTo({
        top: 0,
        behavior: "smooth",
      })
